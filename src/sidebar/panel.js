...
function Panel() {
	...
	const isPublished = useSelect(
		( select ) => select( 'core/editor' ).isCurrentPostPublished(),
		[]
	);
	...
	const create = () => {
		...
		apiFetch( {
			...
			method: 'POST',
			data: { destination: permalink, label: title, post_id: postId },
		} )
			.then( ( res ) => {
				createNotice(
					'success',
					__(
						'QR code created. Download it for printing from OpenQR > QR Codes.',
						'openqr'
					),
					{ type: 'snackbar' }
				);
			...
			.catch( ( err ) => {
				const message =
					err?.message ||
					__( 'Could not create the QR code.', 'openqr' );
		...
		window.openqrEditor?.isConnected
			? [
					...
					codes && codes.length
						...
						: createElement(
								'p',
								{
									className: 'openqr-panel-empty',
									key: 'empty',
								},
								__(
									'No QR codes point at this page yet.',
									'openqr'
								)
						  ),
					isPublished && permalink
						...
						: createElement(
								...
								__(
									'Publish this page first — the QR needs its final address.',
									'openqr'
								)
						  ),
			  ]
			: [
					createElement(
						...
						{ key: 'connect' },
						__(
							'Connect your OpenQR account to create QR codes for this page.',
							'openqr'
						)
					),
					createElement(
						Button,
						{
							key: 'settings',
							variant: 'secondary',
							href: window.openqrEditor?.keysUrl,
							target: '_blank',
							rel: 'noopener',
						},
